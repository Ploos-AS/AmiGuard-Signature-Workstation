...
Internet -> amiguard.ploos.no quarantine
  ...
  -> AmiSandbox -> session.json + events.jsonl + runtime artifacts
...
`Ploos-AS/AmiSandbox` is the required backend whenever ASW claims dynamic malware-analysis evidence. ASW records the exact AmiSandbox build/revision, sample SHA-256, machine profile and retained evidence hashes. Analysis runs default to JIT disabled, guest networking disabled, disposable writable state and no broad writable host filesystem exposure.
